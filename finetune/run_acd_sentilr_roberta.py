# Fine-tuning SentiLARE for aspect category detection on Res14 and Res16.
...
MODEL_CLASSES = {
    'roberta': (RobertaConfig, RobertaForMultiLabelClassification, ABSATokenizer_roberta),
}
...
def compute_metrics(y_true, y_pred, b=1):
    # Compute F1 in ACD task
    common, relevant, retrieved = 0., 0., 0.
    for ix in range(len(y_true)):
        pre = y_pred[ix]
        cor = y_true[ix]
        for iy in range(len(pre)):
            if pre[iy] == 1 and cor[iy] == pre[iy]:
                common += 1
        retrieved += np.sum(pre)
        relevant += np.sum(cor)
        if ix == 1:
            print(cor)
            print(pre)
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.output_mode == "classification":
            preds = np.array([[int(preds[idx][idy] > 0.5) for idy in range(len(preds[idx]))] for idx in range(len(preds))])
        elif args.output_mode == "regression":
            preds = np.squeeze(preds)

        # Calculate F1 on the valid set
        result = compute_metrics(y_true=out_label_ids, y_pred=preds)
...
def test(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        if args.output_mode == "classification":
            preds = np.array(
                [[int(preds[idx][idy] > 0.5) for idy in range(len(preds[idx]))] for idx in range(len(preds))])
        elif args.output_mode == "regression":
            preds = np.squeeze(preds)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test = False):
    ...
    if args.task_name == "acd14":
        processor = AcdProcessor_14()
    else:
        processor = AcdProcessor_16()
