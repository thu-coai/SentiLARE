# Fine-tuning SentiLARE for aspect term / category sentiment classification on Lap14 and Res14.
...
MODEL_CLASSES = {
    'roberta': (RobertaConfig, RobertaForSequenceClassification, ABSATokenizer_roberta),
}
...
def evaluate(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # Calculate accuracy and macro-f1 on the validation set
        result = (preds == out_label_ids).mean()
        f1_result = f1_score(y_true=out_label_ids, y_pred=preds, average="macro")
        ...
        with open(output_eval_file, "w") as writer:
            ...
            logger.info("  acc = %s", str(result))
            logger.info("  f1 = %s", str(f1_result))
            logger.info("  eval_loss = %s", str(eval_loss))
            writer.write("acc = %s\n" % (str(result)))
            writer.write("f1 = %s\n" % (str(f1_result)))
            writer.write("eval_loss = %s" % str(eval_loss))
...
def test(args, model, tokenizer, prefix=""):
    ...
    for eval_task, eval_output_dir in zip(eval_task_names, eval_outputs_dirs):
        ...
        # Calculate accuracy and macro-f1 on the test set
        result = (preds == out_label_ids).mean()
        f1_result = f1_score(y_true=out_label_ids, y_pred=preds, average="macro")
        results.append(result)
        f1_results.append(f1_result)
...
def load_and_cache_examples(args, task, tokenizer, evaluate=False, test = False):
    ...
    # Select the data processor for aspect term / category sentiment classification
    ...
    if os.path.exists(cached_features_file):
        ...
        for ex_index in range(5):
            print(features[ex_index].input_ids)
            print(features[ex_index].input_mask)
            print(features[ex_index].segment_ids)
            print(features[ex_index].pos_ids)
            print(features[ex_index].senti_ids)
            print(features[ex_index].polarity_ids)
            print(features[ex_index].label_id)
    else:
...
def main():
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result, f1_result = evaluate(args, model, tokenizer, prefix=global_step)
            results.extend(result)
            f1_results.extend(f1_result)
        ...
        model.to(args.device)
        result, f1_result = test(args, model, tokenizer, prefix=global_step)
        logger.info("Best valid acc: %.4f" % (max(results)))
        logger.info("Test acc: %.4f" %  (result[0]))
        logger.info("Test f1: %.4f" % (f1_result[0]))
