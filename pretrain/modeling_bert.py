...
class BertEmbeddings(nn.Module):
    ...
    def __init__(self, config, pos_tag_embedding = False, senti_embedding=False, polarity_embedding=False):
        ...
        if senti_embedding:
            ...
            #self.senti_embeddings = nn.init.normal_(nn.Parameter(torch.Tensor(2, config.hidden_size)))
        else:
...
class BertPreTrainingHeads(nn.Module):
    def __init__(self, config, word_embedding, pos_tag_embedding=False, senti_embedding=False, polarity_embedding=False):
        super(BertPreTrainingHeads, self).__init__()
        if pos_tag_embedding[0]:
            self.pos_tag_predictions = BertPostagHead(config, pos_tag_embedding[1])
        else:
            self.register_parameter('pos_tag_predictions', None)
        if senti_embedding[0]:
            self.senti_word_predictions = BertSentiHead(config, senti_embedding[1])
        else:
            self.register_parameter('senti_word_predictions', None)
        if polarity_embedding[0]:
            self.polarity_predictions = BertPolarityHead(config, polarity_embedding[1])
        else:
            self.register_parameter('polarity_predictions', None)
        self.predictions = BertLMPredictionHead(config, word_embedding)
        self.seq_relationship = nn.Linear(config.hidden_size, 2)
    ...
    def forward(self, sequence_output, pooled_output):
        ...
        prediction_scores = self.predictions(sequence_output)
        if self.senti_word_predictions is not None:
            senti_scores = self.senti_word_predictions(sequence_output)
        else:
            senti_scores = None
        if self.pos_tag_predictions is not None:
            pos_tag_scores = self.pos_tag_predictions(sequence_output)
        else:
            pos_tag_scores = None
        if self.polarity_predictions is not None:
            polarity_scores = self.polarity_predictions(sequence_output)
        else:
            polarity_scores = None

        seq_relationship_score = self.seq_relationship(pooled_output)
        return prediction_scores, seq_relationship_score, pos_tag_scores, senti_scores, polarity_scores
...
class BertModel(BertPreTrainedModel):
    ...
    ```
    """
    def __init__(self, config, output_attentions=False, keep_multihead_output=False, pos_tag_embedding=False, senti_embedding=False, polarity_embedding=False):
    ...
    def forward(self, input_ids, token_type_ids=None, pos_tag_ids=None, senti_word_ids=None, polarity_ids=None, attention_mask=None, output_all_encoded_layers=False, head_mask=None):
        ...
        embedding_output = self.embeddings(input_ids, token_type_ids, pos_tag_ids, senti_word_ids, polarity_ids)
...
class BertForPreTraining(BertPreTrainedModel):
    ...
    def __init__(self, config, output_attentions=False, keep_multihead_output=False, pos_tag_embedding=False, senti_embedding=False, polarity_embedding=False):
        ...
        if self.bert.embeddings.pos_tag_embeddings is not None:
            pos_tag_embed_weights = (pos_tag_embedding, self.bert.embeddings.pos_tag_embeddings.weight)
        else:
            pos_tag_embed_weights = (pos_tag_embedding, None)

        if self.bert.embeddings.senti_embeddings is not None:
            senti_embed_weights = (senti_embedding, self.bert.embeddings.senti_embeddings.weight)
            #senti_embed_weights = (senti_embedding, self.bert.embeddings.senti_embeddings)
        else:
            senti_embed_weights = (senti_embedding, None)

        if self.bert.embeddings.polarity_embeddings is not None:
            polarity_embed_weights = (polarity_embedding, self.bert.embeddings.polarity_embeddings.weight)
        else:
    ...
    def forward(self, input_ids, attention_mask=None, token_type_ids=None, masked_lm_labels=None, next_sentence_label=None, pos_tag_ids = None, senti_word_ids=None, polarity_ids=None, pos_tag_labels=None, senti_words_labels=None, polarity_labels=None, head_mask=None):
        outputs = self.bert(input_ids, token_type_ids, pos_tag_ids, senti_word_ids, polarity_ids, attention_mask)
        if self.output_attentions:
        ...
        if masked_lm_labels is not None and next_sentence_label is not None:
            ...
            pos_tag_loss = torch.zeros_like(next_sentence_loss, device = next_sentence_loss.device)
            senti_loss = torch.zeros_like(next_sentence_loss, device = next_sentence_loss.device)
            polarity_loss = torch.zeros_like(next_sentence_loss, device = next_sentence_loss.device)
            ...
            return masked_lm_loss, next_sentence_loss, pos_tag_loss, senti_loss, polarity_loss
...
class BertForSequenceClassification(BertPreTrainedModel):
    ...
    ```
    """
    def __init__(self, config, num_labels=2, output_attentions=False, keep_multihead_output=False, pos_tag_embedding=False, senti_embedding=False, polarity_embedding=False):
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, labels=None, pos_tag_ids=None, senti_word_ids=None, polarity_ids=None, head_mask=None):
        outputs = self.bert(input_ids, token_type_ids, pos_tag_ids, senti_word_ids, polarity_ids, attention_mask, output_all_encoded_layers=False, head_mask=head_mask)
        if self.output_attentions:
            all_attentions, _, pooled_output = outputs
        else:
            _, pooled_output = outputs
